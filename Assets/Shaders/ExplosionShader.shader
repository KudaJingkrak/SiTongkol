...
Shader "Custom/ExplosionShader" {
	Properties
	{
		...
		_Rows ("Rows Count", Int) = 5
		_Frame ("Per Frame Length", Float) = 5
	}
      ...
      Stencil {
        ...
        float frame = fmod(_Time.y / _Frame, frames);
			...
			fixed4 frag (v2f i) : SV_Target {
				...
				// fixed4 c = lerp(shot(_MainTex, i.uv, dx, dy, current), shot(_MainTex, i.uv, dx, dy, next), frame - current) * _Color;
        // if(c.a < 0.3) discard;
        // return c;
      }
